class Instance:
    ...
    def __init__(self, filename):
        ...
        self.library = list(enumerate(self.library))
        self.library.sort(key=lambda x: (self.days - x[1].signup)*x[1].perday, reverse=True)
...
def run_d(instance: Instance):
    start = 0
    choosen_book = set()
    result = []
    for i, library in instance.library:
        if instance.days <= start + library.signup:
            continue
        scanable = instance.days - start - library.signup

        books = set(library.books).difference(choosen_book)
        books = list(map(lambda x: (x, instance.scoring[x]), books))
        books.sort(key=lambda x: x[1], reverse=False)
        books = list(map(lambda x: x[0], books[:scanable]))

        if len(books) == 0:
            continue

        choosen_book = choosen_book.union(set(books))
        result.append((i, books))
        print(len(result), instance.libraries)
    return result
...
if __name__ == '__main__':
    ...
    result = run_d(instance)
    prepare_result(result, 'd_result.out')